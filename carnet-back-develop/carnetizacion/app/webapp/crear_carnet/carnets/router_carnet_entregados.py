...
from db.repository.carnet_activo import lista_entregados_filtrado_por_todos
...
async def carnet_entregados(request: Request, db: Session = Depends(get_db),page: int = 1,filtro1: str = None,filtro2: str = None,filtro3: str = None):
    #print("estoy en carnet Entregados")
    try:
        token = request.cookies.get("access_token")
        scheme, param = get_authorization_scheme_param(token)
            
        carnets = lista_entregados_filtrado_por_todos(db=db, carnet_ci=filtro1, area=filtro2, nombre=filtro3)
        ...
        if filtro1 is not None or filtro2 is not None or filtro3 is not None:
            ...
            if carnets.count() == 0:
                ...
                return templates.TemplateResponse(
                     "general_pages/carnets/carnets_entregados.html", {"request": request,
                                                          "carnets": carnets,
                                                          "persons": persons,
                                                          "motivos":motivos,
                                                          "page_obj":page_obj,
        ...
        raise Exception("Error en carnet entregado"+ e)
