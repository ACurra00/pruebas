...
from api.endpoints.router_imprimir_carnet import print_carnets_solicitados_por_tipo, print_carnets_solicitado, print_carnets_solicitados_por_tipo_fotos
...
from db.repository.carnet_activo import lista_solicitados_filtrado_por_todos
...
async def carnet_solicitado(request: Request, db: Session = Depends(get_db),page: int = 1,filtro1: str = None,filtro2: str = None,filtro3: str = None, imprimir: bool = False, foto: bool = False, carnet_id: str = None):
    try:
        token = request.cookies.get("access_token")
        scheme, param = get_authorization_scheme_param(token)
        current_user: Usuario = get_current_user_from_token(param, db)
        print("El valor de foto es:")
        print(foto)
        print("El valor de imprimir es:")
        print(imprimir)
        if(imprimir):
            if(carnet_id is not None):
                print("Entra al solo")
                return print_carnets_solicitado(db=db, current_user=current_user,  carnet_id=carnet_id)
            if(foto):
                print("Entra al todos como foto")
                return print_carnets_solicitados_por_tipo_fotos(current_user=current_user, db=db)
            else:
                print("Entra al todos como pdf")
                return print_carnets_solicitados_por_tipo(current_user=current_user, db=db)

        '''if(filtro1 != None and len(filtro1)==11  and filtro1 !=""):# filtro para el buscar solo por el carnet
            carnets = lista_solicitado_filtrado_por_ci(db=db,carnet_ci= filtro1)
            persons = list_personas_por_ci(db=db, ci= filtro1)
        elif(filtro2 != None and filtro2 != ""): # filtro para las areas
            carnets = lista_solicitado_filtrado_por_area(db=db,area=filtro2)
            persons = list_personas_por_area(db=db, area= filtro2)
        elif(filtro3 != None and filtro3 != ""):
            carnets = lista_solicitados_filtrado_por_nombre(db=db, nombre=filtro3)
        ...
        '''
        carnets = lista_solicitados_filtrado_por_todos(db=db, carnet_ci=filtro1, area=filtro2, nombre=filtro3)
        ...
        #print("este es el numero de pagina ::")
        ...
        raise Exception("Error en carnet Solicitados", e)
